    ...
    <div className={'flex flex-col gap-4 mx-32'}>
      ...
      {productList && productList.map(product => (
        <div key={product._id} className={''}>
          <ProductCard productImage={product.productImage} productName={product.productName} productDescription={product.productDescription} productPrice={product.productPrice} productUnit={product.productUnit} productLink={`/${product._id}`}/>
        </div>))}
